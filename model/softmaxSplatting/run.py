...
try:
    ...
    sys.path.insert(0, './'); import correlation # you should consider upgrading python
# end
...
class Synthesis(torch.nn.Module):
    def __init__(self):
        ...
        # Combining backFlow and upsampling images with forwardFlow and upsampling images as a new feature vector
...
class SoftmaxSplatting(torch.nn.Module):
    def __init__(self):
        super().__init__()
...
def estimate(tenOne, tenTwo, fltTimes):
    ...
    if netNetwork is None:
        netNetwork = SoftmaxSplatting().cuda().eval()
    # end
